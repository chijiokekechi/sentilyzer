...
### What shipped: `ml/sentilyzer_ml/pipeline/modal_app.py` (verified 2026-08-11)

The trainer that actually shipped is a self-service app, **`sentilyzer-train`**, not the scheduled `sentilyzer-pipeline` sketched below: nothing has a schedule (an untriggered month costs $0), the corpus defaults to the free HackerNews archive mirror streamed straight into a Modal Volume (optionally blended via `--corpus` with the Go harvester's output for the other sources docs/corpus-policy.md admits), and neither R2 nor Neon is in the loop. Aspect distillation shipped end to end (mining, labeling, interleaved KD, a separate gate, and a serving bind); the bullets below carry the details. Where the planned tables in this section disagree with this list, the shipped values govern.
...
- **`--fresh-corpus`.** `reset_corpus()` deletes the corpus documents and tombstones inside the app, and the next ingest rebuilds every month; teacher labels survive (they are keyed per document), which is what keeps the rerun cheap. Contradictory flag combinations (`--fresh-corpus` with `--skip-ingest` or `--corpus`) fail loudly instead of silently ignoring the reset, and `ingest_hn` now `volume.reload()`s before checking `month_done()`: a warm container's pre-reset snapshot would otherwise still see the files the reset just deleted and skip every month as already ingested.
...
- **Aspect labeling.** Candidate aspects are mined from the training set itself by deterministic RAKE-style extraction: pure per-document text statistics, no model, no network, so every pass sees identical candidates and incremental reuse stays honest. Mining is doc-capped at 400k documents BEFORE any pairs are cut (RAKE over the full 1.8M-row corpus was an hour of silent CPU inside a billed GPU container, observed live), and the run labels at most 400k `(text, aspect)` pairs, chosen deterministically by md5 of the pair key so re-runs converge on the same subset. The frozen `yangheng/deberta-v3-base-absa-v1.1` labels them fp32 on a T4 at batch 64, flush-committed every 25k pairs, incremental keyed `(platform, doc_id, aspect, teacher_version)`. The GPU image gained `sentencepiece` + `protobuf`: the ABSA tokenizer requires them, and the first live aspect-labeling run died without them.
- **Training.** The device rides `DistillConfig` end to end (`"cuda"` on Modal), TF32 is enabled, batch 64. All three GPU functions raise if torch sees no CUDA device (the first full run burned its 45 minutes doing 240 steps on CPU). Checkpoints save CPU and CUDA RNG state, so a resumed run reproduces an uninterrupted one. With aspect labels present, KD interleaves: a third of executed steps train the aspect head on `(text, aspect)` text-pair batches (`aspect_fraction=1/3` in the app; the framework default is 0.25, and the measured runs below are why the app passes 1/3). An exhausted aspect stream falls back to doc steps, and the checkpoint carries the interleave cursor so a resumed run lands on the same slot. Memory reservations are explicit: prep 8 GiB, label 4 GiB, label_aspects 4 GiB, train 8 GiB. `run_pipeline` runs under `timeout=8 * 3600`.
- **The aspect gate, and the serving bind.** `evaluate()` runs a separate, looser gate over the held-out pair tail: agreement 0.75, per-class recall 0.40, minimum 300 rows (the aspect head learns from far fewer pairs than the doc head, and a failed gate only keeps aspects on the base backend). The verdict names the exact model it judged: `eval.json` is stamped with the INT8 model's sha256, and serving routes aspect requests to the student only on that bound proof (`aspect_gate_passed` re-hashes the local file and compares). A missing report, a failed gate, or a sha mismatch keeps aspects on the base backend; doc-level serving is unaffected by any of it.
...
- **The run lock is heartbeat-based.** The orchestrator writes `beat_at` into the lock every minute; a holder silent for 300 seconds is presumed dead, and ANY invocation takes the run over: same `run_id`, every stage fast-forwards (prep returns its committed manifest, labels are incremental, train resumes its checkpoint). The `input_id` match survives only as the no-wait fast path, because identity alone proved unreliable in the field: Modal's preemption retry arrived with a DIFFERENT input id, so the retry refused its own run while an orphaned stage kept working. `LOCK_STALE_SECONDS` (3 h) remains the absolute cap no lock may outlive, heartbeats or not.
- **Informed-consent knobs.** `--train-minutes` (hard ceiling 90) and `--aspect-pairs` (hard ceiling 800k) raise the 45-minute and 400k-pair budgets. The ceilings are enforced three times: in `main()` before touching Modal, re-asserted in `run_pipeline` on CPU before any GPU container spawns, and again inside `train()` and `label_aspects_stage()` themselves, so a direct stage invocation that bypassed the consent gate still cannot exceed them. The train stage's Modal `timeout` is sized to the CEILING plus export headroom, so the budget check, never Modal, is what stops a run.
...
- **Measured aspect runs, for the record.** `train:2026-08-11-033757` (aspect_fraction 0.25): doc agreement 0.8676 PASSED, aspect 0.7430 FAILED the 0.75 bar (the quarter of steps covered only ~66% of the pairs inside the budget). `train:2026-08-11-054942` (fraction 1/3): doc 0.8564 PASSED, aspect 0.7555 PASSED. Both ran on a blended corpus of 1,832,780 HN + 7,220 Bluesky rows, with 324 rows dropped by live delete-tombstones.
- **Tests.** `ml/tests` is 99 tests; `make e2e` runs 14 local end-to-end checks (stub worker + gateway, every protocol and format asserted) in about a minute, no GPU and no Modal account required.
...
*(Planned shapes. The shipped app's guards differ where the "What shipped" list above says so: `label` and `label_aspects` each at 4 h / 4 GiB with one orchestrator-level retry on `FunctionTimeoutError`, `train` at 8 GiB with its Modal timeout sized to the 90-minute `--train-minutes` ceiling plus export headroom, `run_pipeline` at 8 h.)*
...
8. **How many aspect pairs does real traffic actually produce?** Aspect supervision comes from topic-scoped harvest `(text, topic)` plus user-requested `Aspects`; the firehose contributes doc-level only. If topic-scoped volume is thin, the `n_aspect_pairs >= 5,000` gate fails and `head_aspect` is starved. **Only real traffic answers this.** Fallback (entity/ticker regex mining) adds noise and should **not** be pre-built. If ABSA defers to v2, the teacher drops to ~$0.60/mo and the CPU-vs-GPU question becomes entirely moot. **Resolved by what shipped (2026-08-11): traffic is out of the loop entirely.** Candidate aspects are mined from the corpus itself by deterministic RAKE-style extraction (pure per-document text statistics, so every pass sees identical candidates and incremental labeling stays honest), doc-capped at 400k documents and pair-capped at 400k. The starved case is handled by the aspect gate's 300-row minimum, and a failed gate only keeps aspects on the base backend.
...
11. **Does the two-head single-ONNX export actually work?** The spec assumes one graph, inputs `(input_ids, attention_mask)`, outputs `(logits_doc, logits_aspect)`, dynamic axes. Verify `optimum-onnx` exports a custom two-head module cleanly, and that `onnxruntime.transformers.optimizer` (which matters **specifically because** of dynamic axes) doesn't mangle it. **Fallback:** two ONNX files with a duplicated encoder (~2× disk/RAM, still small). **Resolved (2026-08-11): yes.** The shipped `TwoHeadStudent` exports as one INT8 graph with exactly that signature; aspects ride the tokenizer's text-pair encoding, `OnnxBackend` serves both heads from the single file, and the gate-passing run `train:2026-08-11-054942` was evaluated through it. No fallback needed.
